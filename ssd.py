...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        #print("Size of network is {}.".format(size))
    ...
    def load_into_partial_from_original_network(self, base_file, loadUpTo):
        other, ext = os.path.splitext(base_file)
        if ext == '.pkl' or '.pth':
            print('Loading weights into state dict...')
            allWeights = torch.load(base_file,
                                 map_location=lambda storage, loc: storage)
            own_state = self.state_dict()
            state_dict = allWeights
            for name, param in state_dict.items():
                layerNum = extract_nbr(name)
                if name not in own_state:
                    self.load_state_dict(own_state)
                    print('Finished!') 
                    return 
                else:
                    if layerNum < loadUpTo:
